...
function miniloop_image( $atts ) {
	...
	switch ($from) {
		case 'thumb' :
			...
			delete_post_meta( $post->ID, '_ml_thumb_thumb' );
			...
			elseif ( has_post_thumbnail( $post->ID ) ) {
				...
				$src = miniloops_create_thumbnail( $id, $width, $height );
				//save to meta
			...
			if ( isset( $resized["{$width}x{$height}"] ) ) {
				...
				foreach( $atts as $a ) {
					...
					if ( strpos( $a->post_mime_type, 'image/' ) !== false ) {
						$src = miniloops_create_thumbnail( $a->ID, $width, $height );
						//save to meta
						$resized["{$width}x{$height}"] = $src;
...
function miniloops_create_thumbnail( $att_id, $width, $height ) {
//	die( content_url() );
	...
	$src = str_replace( WP_CONTENT_DIR, content_url(), $new );
